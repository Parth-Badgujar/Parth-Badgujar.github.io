...
date: 2023-12-21 23:00:00 +0530
categories: [pwn]
tags: [pwn, heap, buffer-overflow]     # TAG names should always be lowercase
---
A jeopardy-style CTF conducted by InfoSecIITR. The CTF will consist of challenges from various categories such as Web, Blockchain, Pwn, Reversing, Cryptography, and Forensics. 
...
The program asks for a `key` and matches it with randomly generated key, we can predict the randomly generated key since it is using `srand(time(0))` but it is not the goal. Given two chances, the goal is to leak the `canary` from first input and overflow the return address from the second input as both of them are using `scanf("%s", buf)` format specifier to read input. 

### Leaking Canary (first input) 
`printf("%s", buf)` or `puts(buf)` both will print characters untill the encounter a `null byte`. The actual canary is also stored on the stack at the bottom, which is then matched with canary from `gs`, so if we give an input such that its length reaches upto the canary in the stack, along with overwriting null byte of the canary, in the `printf` call it will print out input along with the canary as there is no null byte between out input and canary.  
...
### House of Botcake  
This attack allows us to call sort of double free, where we make overlapping chunks one in `unsorted bin` ovverlapped with chunk in `tcache bin`. 

Attack sequence : 
1. Fill tcache of say 0x90 size by allocating and freeing 7 chunks of that size. 
2. Allocated 3 more chunks along with thoose 7, one of it will be allocated to prevent to prevent consolidation of other chunks with top chunk. 
3. Say our allocation sequence is 7 tcache chunks + chunk A + chunk B + extra chunk.
4. So first free chunk B which will go in `unsorted bin`, then free chunk A which will consolidate with chunk B in `unsorted bin`, then make space for chunk B in tacache by allocating one more chunk, then free chunk B. Thus double freeing chunk B. 
5. Allocate 0x120 which will be allocated in chunk (A + B), we'll call it chunk C. 
5. Now we can overwrite `fd` pointer of chunk B by editing the data in chunk C. 
6. Since we have control of `fd` pointer of chunk B we can allocte chunk at any address. 

### Attack 
This binary asks us for different toppings, which act like list indexs in a list of heap pointers, there we 10 toppings, which means 10 indexes.

We first perform the attack sequence to allocate a chunk on libc `environ` pointer to read a stack address, at the same time viewing unsorted bin chunk to leak libc address before allocating chunk. 
...
Then we perform the attack again to allocate chunk on stack and write ROP chain on it. 
